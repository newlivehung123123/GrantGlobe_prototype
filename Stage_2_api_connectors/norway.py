#!/usr/bin/env python3
"""
Research Council of Norway (RCN) connector — Stage 2 API source.

Scrapes open funding calls from forskningsradet.no.
No API key required. Server-rendered HTML.

Usage (on the VPS):
    export $(grep DATABASE_URL /opt/grantglobe/Stage_3_LLM_extraction/.env | xargs)
    python3 Stage_2_api_connectors/norway.py [--dry-run]
"""
...
DOMAIN_SECTOR_MAP: dict[str, list[str]] = {
    "life science":      ["Health Sciences", "Agriculture & Food"],
    "health":            ["Health Sciences"],
    "energy":            ["Climate & Environment", "Science & Technology"],
    "environment":       ["Climate & Environment"],
    "marine":            ["Climate & Environment", "Agriculture & Food"],
    ...
    "petroleum":         ["Climate & Environment", "Science & Technology"],
...
def _parse_calls_from_html(html_text: str) -> list[dict]:
    """
    Extract open calls from RCN listing HTML.

    Looks for links whose href contains '/apply-for-funding/' or '/funding/'
    that point to individual call detail pages.
    """
    opps: list[dict] = []
...
def _fetch_rcn_opportunities() -> list[dict]:
    ...
    for page_num in range(1, MAX_PAGES + 1):
        url = RCN_LIST_URL if page_num == 1 else f"{RCN_LIST_URL}?page={page_num - 1}"
        html_text = _fetch_page(session, url)
        if not html_text:
            print(f"  RCN: page {page_num} returned no content — stopping.")
            break
        ...
        if not page_opps:
            print(f"  RCN: page {page_num} yielded 0 opportunities — stopping.")
            break

        existing_urls = {o["url"] for o in all_opps}
        new_opps = [o for o in page_opps if o["url"] not in existing_urls]
        all_opps.extend(new_opps)
        print(f"  RCN page {page_num}: {len(new_opps)} new opportunities (total: {len(all_opps)})")
...
def _map_opportunity(opp: dict) -> dict | None:
    ...
    return {
        ...
        "applicant_base_countries": ["NO"],
        "geographic_focus_countries": [],
        "organisation_types":       [],
        "individual_eligibility":   [],
        "domain":                   "api_rcn_norway",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Research Council of Norway → GrantGlobe ingestor")
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()

    print("Fetching Research Council of Norway open calls …")
    raw_opps = _fetch_rcn_opportunities()
