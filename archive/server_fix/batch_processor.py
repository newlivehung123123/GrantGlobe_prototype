...
def _meta_crawl_date(meta: dict) -> str:
    """Return the crawl date for a meta dict as an ISO YYYY-MM-DD string.

    Stage 2 writes ``crawl_timestamp`` (full ISO-8601 datetime) rather than
    the ``crawl_date`` field this module originally expected.  Accept either:
    prefer an explicit ``crawl_date``, otherwise take the first ten
    characters of ``crawl_timestamp``.
    """
    return str(meta.get("crawl_date") or (meta.get("crawl_timestamp") or "")[:10])
...
def scan_for_pending_pages(
    ...
    with get_cursor(conn) as cur:
        for meta_path in sorted(raw_cache_path.rglob("*.meta.json")):
            ...
            if _meta_crawl_date(meta) != date_str:
                continue
...
def _count_tokens(text: str, model_name: str) -> int:
    """Estimate the token count for *text* as ~4 characters per token.

    A local estimate replaces the previous per-page ``count_tokens`` API
    round-trip, which added seconds of latency per page and depended on the
    deprecated google-generativeai package.  The 4-chars-per-token heuristic
    is conservative for English web text and adequate for the two purposes
    this function serves: the 50-token minimum-content floor and the
    6 000/12 000-token truncation budgets.
    """
...
def prepare_html_page(
    ...
    # -- locate the stored HTML file -----------------------------------------
    # Stage 2's ContentStoragePipeline writes the page body as
    # ``{url_hash}.html`` NEXT TO the ``{url_hash}.meta.json`` sidecar, and
    # the bytes are GZIP-COMPRESSED despite the ``.html`` extension
    # (content_storage.py: ``gzip.compress(body)`` → ``{url_hash}.html``).
    # The meta sidecar carries no ``html_path`` field, so prefer it when
    # present (forward compatibility) and otherwise infer the sibling path.
    html_path_rel: Optional[str] = meta.get("html_path")
    if html_path_rel:
        html_file = raw_cache_dir / html_path_rel
    else:
        html_file = meta_json_path.with_name(
            meta_json_path.name.replace(".meta.json", ".html")
        )

    if not html_file.exists():
        log.error("html_file_not_found", path=str(html_file))
        return None, "html_file_not_found"

    # -- read and (if necessary) decompress ----------------------------------
    try:
        blob: bytes = html_file.read_bytes()
    except OSError as exc:
        log.error("html_read_error", path=str(html_file), error=str(exc))
        return None, "html_read_error"

    # Gzip magic number: 0x1f 0x8b.  Decompress when present; fall through to
    # a plain decode otherwise, so both storage formats are handled.
    if blob[:2] == b"\x1f\x8b":
        try:
            blob = gzip.decompress(blob)
        except (gzip.BadGzipFile, EOFError, OSError) as exc:
            log.error("html_gunzip_error", path=str(html_file), error=str(exc))
            return None, "html_read_error"

    raw_html = blob.decode("utf-8", errors="replace")
...
def _build_meta_index(
    ...
    """Walk raw_cache_dir and return a mapping of (url_hash, crawl_date) → meta_json_path.

    Indexes every meta file regardless of crawl date.  Claimed
    ``extraction_log`` rows carry their own ``crawl_date`` (the date they were
    registered under), which may differ from the cycle's ``run_date`` when a
    previous cycle's batch left rows pending — those rows must still be looked
    up against their own crawl date's meta files, not the current run_date's.
    """
    ...
    for meta_path in Path(raw_cache_dir).rglob("*.meta.json"):
        try:
            ...
            continue
        url_hash: str = meta.get("url_hash") or Path(meta_path.stem).stem
        crawl_date = _meta_crawl_date(meta)
        index[(url_hash, crawl_date)] = str(meta_path)
    return index
...
def run_extraction_cycle(
    ...
    # Step 3 — build (url_hash, crawl_date) → meta_path index (fast filesystem walk)
    meta_index = _build_meta_index(raw_cache_dir)
    ...
    for row in claimed_rows:
        ...
        row_crawl_date = str(row.get("crawl_date") or run_date)
        meta_path_str = meta_index.get((url_hash, row_crawl_date))
    ...
    # Step 6 — submit to extraction function (synchronous Gemini or mock)
    ...
    try:
        ...
        try:
            ...
            # Per-page writes are committed individually inside extract_pages;
            # a connection that died during a multi-hour run must not
            # invalidate work that is already durable.
            log.warning("final_commit_skipped_connection_dead")
    except Exception as exc:
        log.error("extraction_failed", error=str(exc))
        # Release ONLY rows still claimed by this pass back to 'pending' so
        # the next cycle retries them automatically.  Rows already marked
        # completed/failed by per-page commits keep their status — never
        # overwrite finished (and paid-for) work.
        conn2 = get_connection()
        try:
            ids = [item["row"]["id"] for item in page_contents]
            with get_cursor(conn2) as cur:
                cur.execute(
                    """
                    UPDATE extraction_log
                    SET status = 'pending',
                        error_message = %(error)s
                    WHERE id = ANY(%(ids)s) AND status = 'processing'
                    """,
                    {"ids": ids, "error": f"pass_aborted: {exc}"},
                )
                log.warning("pass_aborted_rows_released", count=cur.rowcount)
        finally:
            conn2.close()
        raise
    finally:
