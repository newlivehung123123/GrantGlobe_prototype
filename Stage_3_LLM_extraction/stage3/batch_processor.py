...
def scan_for_pending_pages(
    ...
    with get_cursor(conn) as cur:
        for meta_path in sorted(raw_cache_path.rglob("*.meta.json")):
            ...
            if str(meta.get("crawl_date", "")) != date_str:
                continue
...
def _count_tokens(text: str, model_name: str) -> int:
    """Return the Gemini token count for *text* using the target model.

    Imported lazily so that importing batch_processor does not fail when
    google-generativeai is absent (e.g. during unit tests that mock this
    function directly).
    """
